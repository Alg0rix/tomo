...
def _map_event(
    ...
    chunks with increasing ids and returns the new last-seq. Each history
    entry matches the ``ChatEntry`` replay shape (type/content/agent_id/
    function/params/error/ts). Unknown event kinds are a no-op so a future
    loop event can't crash the stream.
    """
    kind = ev["kind"]
    chunks: list[str] = []
...
async def stream_turn_sse(
    ...
    a matching ``append_session_history`` entry via :func:`_map_event`.
    ``state`` busy true/false and ``turn.start`` are always emitted; a missing
    agent is surfaced as an SSE ``error`` so the stream stays well-formed.
    The trailing busy-false ``state`` fires from a ``finally`` so it always
    runs, even if the loop or persistence raises.
    """
    seq = start_seq
    try:
        ...
        if not agent:
            ...
            async for ev in _agent_run_turn(
                ...
                chunks, entries, seq = _map_event(ev, coordinator_id, agent_name, seq)
                for chunk in chunks:
                    yield chunk
