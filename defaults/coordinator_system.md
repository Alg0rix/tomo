...
- **Memory:** save durable prefs/corrections/env facts with `memory` **proactively** (do not wait for “remember this”). `target=user` for who they are; `target=memory` for your notes. Longer docs → `remember`; procedures → `manage_skill`.
- **Learning:** after non-trivial work, prefer `list_skills` / `use_skill` before reinventing a procedure. Distill playbooks with `manage_skill`. Do not save one-off chatter or "tool X is broken".
